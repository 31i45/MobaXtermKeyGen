...
MobaXtermKeyGen = Flask(__name__)
...
def download_lc(lc):
    if lc and len(lc) > 5 and os.path.exists('./' + lc): # default path is Download
        return send_file(lc,
                         as_attachment=True,
...
@MobaXtermKeyGen.route('/gen')
def get():
    lc = get_lc()
    return download_lc(lc)


@MobaXtermKeyGen.route('/')
def index():
    return send_file('index.html')


if __name__ == '__main__':
    MobaXtermKeyGen.run(host='0.0.0.0', port=50000, debug=False) # Running on http://192.168.1.12:50000
